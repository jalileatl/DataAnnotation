...
# New directory for saved images
SAVED_IMAGES_DIR = "saved_images"
if not os.path.exists(SAVED_IMAGES_DIR):
    os.makedirs(SAVED_IMAGES_DIR)

# Metadata file for saved images
METADATA_FILE = os.path.join(SAVED_IMAGES_DIR, "metadata.json")

def load_metadata():
    """Load metadata for saved images."""
...
def save_metadata(metadata):
    """Save metadata for saved images."""
...
def save_image_to_library(image_data, metadata):
    """Save image with annotations to the library."""
    image_id = str(uuid.uuid4())[:8]
    image_file = os.path.join(SAVED_IMAGES_DIR, f"{image_id}.png")
...
def load_image_from_library(image_id):
    """Load image from the library."""
    image_file = os.path.join(SAVED_IMAGES_DIR, f"{image_id}.png")
    metadata = load_metadata()
    
    if not os.path.exists(image_file) or image_id not in metadata:
        return None, None
    ...
    return image_b64, metadata[image_id]
...
def chat_with_openai(prompt, history, image_data=None, api_key=OPENAI_API_KEY):
    ...
    try:
        ...
        Always classify potential diseases in the image and provide tailored recommendations based on the classification.
        Provide helpful analysis, explanations, and educational insights.
        ...
        1. Summary (including disease classification)
        2. Observations
        3. Recommendations (specific to the classified disease, e.g., treatment options, further tests)
        4. Teaching Points
        ...
        dbc.Row([
            dbc.Col(id="left-column", md=9, children=[
                dbc.Card([
                    ...
                    dbc.CardBody([
                        # Modified upload area with two buttons
                        html.Div([
                            dbc.ButtonGroup([
                                dbc.Button(
                                    [html.I(className="bi bi-cloud-upload me-2"), "Upload New Image"],
                                    id="upload-new-btn",
                                    color="primary",
                                    outline=True,
                                    className="me-2"
                                ...
                                dbc.Button(
                                    [html.I(className="bi bi-folder2-open me-2"), "Load from Library"],
                                    id="load-library-btn",
                                    color="primary",
                                    outline=True
                                )
    ...
    style={"fontSize": "0.8rem"}
                                ...
                                html.Div(
                                    ...
                                    children=[
                                        ...
                                        dbc.Button(
                                            [html.I(className="bi bi-save me-1"), "Save to Library"],
                                            id="save-library-btn",
                                            color="success",
                                            outline=True,
                                            size="sm",
                        ...
                        html.Div(id="library-status", className="mt-2")
                    ], style={"padding": "1rem"})
        ...
        # Hidden div to contain all stores
        html.Div(style={"display": "none"}, children=[
            dcc.Store(id="canvas-data-store", data=""),
            dcc.Store(id="image_b64-store", data=""),
            dcc.Store(id="filename-store", data=""),
            dcc.Store(id="disp_size-store", data=(500, 500)),
            dcc.Store(id="file_type-store", data=""),
            dcc.Store(id="text-color-store", data="#FF0000"),
        ]),
        dbc.Modal([
        ...
        dbc.Modal([
            dbc.ModalHeader(dbc.ModalTitle("Image Library", style={"fontSize": "1rem"})),
            dbc.ModalBody([
                html.Div(id="library-content")
            ], style={"maxHeight": "70vh", "overflowY": "auto"}),
            dbc.ModalFooter([
                dbc.Button("Close", id="close-library-modal", color="secondary", size="sm")
            ])
        ], id="library-modal", is_open=False, centered=True, size="xl")
    ], fluid=True),
...
# Callback to toggle upload area visibility
@app.callback(
    Output("upload-dicom", "style"),
    Input("upload-new-btn", "n_clicks"),
    prevent_initial_call=True
)
def toggle_upload_area(n_clicks):
    if n_clicks:
        return {
            'width': '100%', 'height': '60px', 'lineHeight': '60px',
            'borderWidth': '2px', 'borderStyle': 'dashed', 'borderRadius': '8px',
...
# Callback to open library modal
@app.callback(
    Output("library-modal", "is_open"),
    Output("library-content", "children"),
    Input("load-library-btn", "n_clicks"),
    Input("close-library-modal", "n_clicks"),
    State("library-modal", "is_open"),
    prevent_initial_call=True
)
def toggle_library_modal(load_clicks, close_clicks, is_open):
    ...
    if button_id == "close-library-modal":
        return False, dash.no_update
    
    if button_id == "load-library-btn":
       
        metadata = load_metadata()
        
        if not metadata:
            return True, html.Div([
                html.H5("No saved images", className="text-center mt-4"),
                html.P("Save images to your library to see them here.", className="text-center text-muted")
            ])
        
        # Create cards for each saved image
        image_cards = []
        for image_id, img_metadata in metadata.items():
            image_file = os.path.join(SAVED_IMAGES_DIR, f"{image_id}.png")
            if os.path.exists(image_file):
                # Create thumbnail if it doesn't exist
                thumbnail_path = os.path.join(SAVED_IMAGES_DIR, f"thumb_{image_id}.png")
                ...
                thumbnail_b64 = pil_to_b64(Image.open(thumbnail_path))
                
                card = dbc.Card([
                    dbc.CardImg(src=thumbnail_b64, top=True, style={"height": "150px", "objectFit": "cover"}),
                    dbc.CardBody([
                        html.H6(img_metadata.get("filename", "Unknown"), className="card-title", style={"fontSize": "0.9rem"}),
                        html.P([
                            html.Small(f"Saved: {img_metadata.get('save_date', 'Unknown')}", className="text-muted d-block"),
                            html.Small(f"Size: {img_metadata.get('dimensions', 'Unknown')}", className="text-muted d-block"),
                            html.Small(f"Type: {img_metadata.get('file_type', 'Unknown')}", className="text-muted d-block"),
                        ]),
                        dbc.Button("Load", id={"type": "load-image-btn", "index": image_id}, 
                                  color="primary", size="sm", className="w-100")
                    ], style={"padding": "0.75rem"})
                ], className="mb-3", style={"maxWidth": "200px"})
        ...
        return True, html.Div([
            dbc.Row([
                dbc.Col([
                    html.H4("Your Image Library", className="mb-4"),
                    html.Div(image_cards, className="d-flex flex-wrap")
                ])
            ])
        ])
...
# Callback to save image to library
@app.callback(
    Output("library-status", "children"),
    Input("save-library-btn", "n_clicks"),
    State("image_b64-store", "data"),
    State("filename-store", "data"),
    State("disp_size-store", "data"),
    State("file_type-store", "data"),
    prevent_initial_call=True
)
def save_to_library(n_clicks, image_b64, filename, disp_size, file_type):
    if not n_clicks:
        return dash.no_update
    
    if not image_b64:
        return dbc.Alert("No image to save. Please upload an image first.", color="warning", dismissable=True, duration=3000)
    ...
    try:
        ...
        metadata = {
            ...
            "dimensions": f"{disp_size[0]}x{disp_size[1]}" if disp_size else "Unknown",
            "file_type": file_type or "Unknown",
            "has_annotations": bool(state["canvas_json_data"]),
        ...
        image_id = save_image_to_library(image_b64, metadata)
        
        return dbc.Alert(
            [html.I(className="bi bi-check-circle me-2"), f"Image saved to library with ID: {image_id}"],
            color="success",
            dismissable=True,
            duration=3000
        )
        ...
        return dbc.Alert(
            [html.I(className="bi bi-exclamation-triangle me-2"), f"Error saving to library: {str(e)}"],
            color="danger",
            dismissable=True,
            duration=5000
        )

# Callback to load image from library
# Callback to load image from library
@app.callback(
    ...
    Output("image_b64-store", "data", allow_duplicate=True),
    Output("filename-store", "data", allow_duplicate=True),
    Output("disp_size-store", "data", allow_duplicate=True),
    Output("file_type-store", "data", allow_duplicate=True),
    Input({"type": "load-image-btn", "index": dash.ALL}, "n_clicks"),
    prevent_initial_call=True
)
def load_image_from_library(clicks):
    ctx = callback_context
    if not ctx.triggered:
    ...
    if not button_id or not isinstance(button_id, dict) or button_id.get('type') != 'load-image-btn':
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    
    image_id = button_id.get('index')
    if not image_id:
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    
    try:
        # Load image and metadata
        image_b64, metadata = load_image_from_library(image_id)
        
        if not image_b64:
            return dash.no_update, dash.no_update, dash.no_update, "Error loading image", {"display": "none"}, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
        ...
        # Create the image_b64 for display
        image_b64_display = pil_to_b64(img_disp)
        ...
        state.update({
            ...
            "flip_horizontal": metadata.get("flip_horizontal", False),
            "flip_vertical": metadata.get("flip_vertical", False),
            "image_b64": image_b64_display,
            "filename": metadata.get("filename", "Unknown"),
            "file_type": metadata.get("file_type", "Unknown"),
            "eraser_strokes": []
        })
        
        info = f"✅ Loaded: {metadata.get('filename', 'Unknown')} | Type: {metadata.get('file_type', 'Unknown')} | Size: {img_orig.size[0]}x{img_orig.size[1]}"
        navigator_style = {"display": "none"}
        
        # Silent analysis upon load
        auto_prompt = "Classify the type of medical image (e.g., X-ray, MRI, ultrasound, photograph), the body part or condition shown, and analyze this medical image for educational purposes, identifying any potential diseases and providing recommendations."
        reply = chat_with_openai(auto_prompt, state["chat_history"], image_b64_display)
        
        state["chat_history"].append({"role": "user", "content": auto_prompt})
        ...
        return image_b64_display, disp_size[0], disp_size[1], info, navigator_style, [], cases_html, image_b64_display, metadata.get("filename", "Unknown"), disp_size, metadata.get("file_type", "Unknown")
    except Exception as e:
        return dash.no_update, dash.no_update, dash.no_update, f"❌ Error: {e}", {"display": "none"}, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update

# Callback to close library modal after loading an image
@app.callback(
    Output("library-modal", "is_open", allow_duplicate=True),
    Input({"type": "load-image-btn", "index": dash.ALL}, "n_clicks"),
    prevent_initial_call=True
)
def close_library_modal_after_load(clicks):
    ctx = callback_context
    if not ctx.triggered:
        return dash.no_update
    ...
    # Close the modal after loading an image
    return False
...
def save_annotation_state(n_clicks, json_data, height, width):
    ...
    if not state["image_orig"]:
        return dbc.Alert(" No image loaded. Please upload an image file first.", color="warning", dismissable=True, duration=3000)
...
def update_zoom(zoom_in_clicks, zoom_out_clicks, fit_clicks):
    ...
    return f"Zoom: {int(state['zoom_level']*100)}%", canvas_style, canvas_width, canvas_height, left_md, right_md, right_style, state["disp_size"]
...
@app.callback(
    ...
    Output("chat-box", "children", allow_duplicate=True),
    Output("radiopaedia-cases", "children", allow_duplicate=True),
    Output("image_b64-store", "data"),
    Output("filename-store", "data"),
    Output("disp_size-store", "data"),
    Output("file_type-store", "data"),
...
def load_image(contents, filename):
    if not contents:
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update, {"display": "none"}, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    try:
        ...
        if file_ext in ['png', 'jpg', 'jpeg']:
            ...
            return dash.no_update, dash.no_update, dash.no_update, f"❌ Unsupported file type: {file_ext}", {"display": "none"}, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
        ...
        # Silent analysis upon upload
        auto_prompt = "Classify the type of medical image (e.g., X-ray, MRI, ultrasound, photograph), the body part or condition shown, and analyze this medical image for educational purposes, identifying any potential diseases and providing recommendations."
        image_data = state["image_b64"]
        reply = chat_with_openai(auto_prompt, state["chat_history"], image_data)
        ...
        return state["image_b64"], disp_size[0], disp_size[1], info, navigator_style, [], cases_html, state["image_b64"], filename, disp_size, state["file_type"]
    except Exception as e:
        return dash.no_update, dash.no_update, dash.no_update, f"❌ Error: {e}", {"display": "none"}, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update
...
def add_text_annotation(n_clicks, text_value, font_size, font_style, x_pos, y_pos, text_color):
    if n_clicks and text_value and state["image_orig"]:
        ...
        return state["image_b64"], state["image_b64"]
    return dash.no_update, dash.no_update
...
def update_tool_buttons(rect_clicks, circle_clicks, line_clicks, pen_clicks, eraser_clicks, text_clicks):
    ctx = callback_context
    if not ctx.triggered:
...
def update_canvas_tool(rect_clicks, circle_clicks, line_clicks, pen_clicks, eraser_clicks, text_clicks, 
                      cancel_clicks, add_clicks, is_open):
    ctx = callback_context
    if not ctx.triggered:
        return "rectangle", False, "red", 3
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    
    if button_id == "tool-text":
        return "rectangle", True, "red", 3
    ...
    tool = tool_map.get(button_id, "rectangle")
...
@app.callback(
    Output("text-color-store", "data", allow_duplicate=True),
...
def handle_chat_and_analysis(send_click, analyze_click, text_value, json_mask, chat_children, height, width, eraser_strokes, image_b64):
    ctx = callback_context
    if not ctx.triggered:
    ...
    if trigger == "send-chat":
        ...
        reply = chat_with_openai(prompt, state["chat_history"], image_b64)
        ...
        if not state["image_orig"]:
            children.append(html.Div([
                html.Strong("Assistant:"),
                html.P(" Upload an image file first.", style={"fontSize": "0.85rem"})
            ], className="p-2 mb-2 bg-primary text-white rounded"))
            return children, "", cases_html
        ...
        if mask_array is None or not mask_array.any():
            ...
            if not bbox_disp:
                children.append(html.Div([
                    html.Strong("Assistant:"),
                    html.P(" Draw a valid annotation first.", style={"fontSize": "0.85rem"})
                ], className="p-2 mb-2 bg-primary text-white rounded"))
                return children, "", cases_html
            ...
            if not cropped:
                children.append(html.Div([
                    html.Strong("Assistant:"),
                    html.P(" Failed to crop region.", style={"fontSize": "0.85rem"})
                ], className="p-2 mb-2 bg-primary text-white rounded"))
                return children, "", cases_html
